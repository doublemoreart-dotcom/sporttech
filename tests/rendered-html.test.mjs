...
test("documents the local and git version boundary", async () => {
  ...
  assert.match(readme, /source main 已是新版但 `gh-pages` 還舊/);
  assert.match(readme, /`gh-pages` 已是新版但 main-site raw 還舊/);
  assert.match(readme, /不要只看 HTTP 200/);
  assert.match(readme, /主站 workflow 會從 `doublemoreart-dotcom\/sporttech:gh-pages` 同步/);
  ...
  assert.match(diagnosePublicSite, /Source repo gh-pages \/index\.html/);
  assert.match(diagnosePublicSite, /Main-site raw \/sporttech\/index\.html/);
  assert.match(diagnosePublicSite, /Live public URL/);
  assert.match(diagnosePublicSite, /source main is current, but sporttech gh-pages is stale/);
  assert.match(diagnosePublicSite, /source repo and gh-pages are current, but main-site repo has not been synced/);
  ...
  assert.match(readme, /SportTech Pages 推版/);
  assert.match(readme, /主站自動同步流程會讀這個分支/);
  ...
  assert.match(readme, /npm run sync:main-site/);
  assert.match(readme, /npm run deploy:pages/);
  ...
  assert.match(updateFlow, /dinopeng\.com\/sporttech\/ updates only after publishing sporttech gh-pages, then syncing and pushing the main-site repo/);
  assert.match(updateFlow, /doublemoreart-dotcom\/sporttech\/tree\/gh-pages/);
  ...
  assert.match(updateFlow, /make sure source main is committed and pushed/);
  assert.match(updateFlow, /npm run deploy:pages to update doublemoreart-dotcom\/sporttech:gh-pages/);
  assert.match(updateFlow, /fast-forward the main-site repo/);
  assert.match(updateFlow, /run npm run status:public to locate any source\/gh-pages\/main-site\/live mismatch/);
...
test("keeps the workbench and lane cards vertically sequenced", async () => {
  ...
  assert.match(css, /@media \(max-width: 700px\)\s*\{[\s\S]*\.site-header nav\s*\{[\s\S]*flex-wrap: nowrap/);
  assert.match(css, /@media \(max-width: 700px\)\s*\{[\s\S]*\.site-header nav\s*\{[\s\S]*overflow-x: auto/);
  assert.match(css, /@media \(max-width: 700px\)\s*\{[\s\S]*\.site-header a:not\(\.site-logo\)\s*\{[\s\S]*white-space: nowrap/);
