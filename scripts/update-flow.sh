...
Steps:
  ...
  3. Optionally deploy to GitHub Pages with --deploy.
  4. Print git status and canonical URLs.
HELP
      exit 0
      ;;
    *)
...
echo "Step 1/4: run checks"
npm run check
echo

echo "Step 2/4: sync local static snapshots"
npm run sync
echo

if [[ "${deploy}" == true ]]; then
  echo "Step 3/4: deploy GitHub Pages"
  npm run deploy:pages
else
  echo "Step 3/4: skip deploy"
  echo "  Run npm run update:deploy when the synced version is ready to publish."
fi
echo

echo "Step 4/4: review git status"
git status --short
echo
